...
function Optimize-Disks {
  ...
  if ($cleanSelection -eq "wspurge" -or $cleanSelection -eq "all") {
    $jobName = Wait-Animation { $(cmd.exe /c "net stop WSearch"; cmd.exe /c "RD /S /Q 'C:\ProgramData\Microsoft\Search'"; Remove-Item -ErrorAction Ignore -Path 'HKCU:\Software\Microsoft\Windows Search' -Recurse -Force; Remove-ItemProperty -Path 'HKLM:\Software\Microsoft\Windows Search' -Name 'SetupCompletedSuccessfully' -Force) } "Cleaning & Reinitializing Windows Search..."
